...
export class beforeThatEnd implements ValidatorConstraintInterface {
  validate(
    ...
    const response = compareAsc(
      (validationArguments.object as MakeBookingDto).check_out_date,
      value,
    );
    return response == 1;
  }
...
export class afterForOneDay implements ValidatorConstraintInterface {
  validate(
    ...
    const check_in_date = (validationArguments.object as MakeBookingDto)
      .check_in_date;
    const diference = formatDistanceStrict(check_in_date, value, {
      unit: 'day',
    });
...
export class minToday implements ValidatorConstraintInterface {
  validate(
    ...
    const currentDateTimeInBogota = new Date(
      currentUTC.getTime() + offset * 60 * 60 * 1000,
    );
    const formattedDateTime = currentDateTimeInBogota
      .toISOString()
      .split('T')[0]
      .split('-')
      .map((e) => Number(e));
    const dateNowBogota = new Date(
      formattedDateTime[0],
      formattedDateTime[1] - 1,
      formattedDateTime[2],
    );
